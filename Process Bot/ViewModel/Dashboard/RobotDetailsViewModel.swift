//
//  RobotDetailsViewModel.swift
//  Process Bot
//
//  Created by DIPIKA GHOSH on 10/08/21.
//

import Foundation

protocol RobotDetailsViewModelProtocol : class {
    ...
    var arrayLogHistory:[robitDetailsModel]{get}
    func callInAction(completion:@escaping (ProcessBot<Any?>)-> Void)
}
class RobotDetailsViewModel : RobotDetailsViewModelProtocol {
    var manager: RequestManager?
    var arrayLogHistory = [robitDetailsModel]()
    func callLogHistoryRobotDetails(triggeredId:String?, completion:@escaping (ProcessBot<Any?>)-> Void){
            ...
            case .success(let jsonresponce):
                if let dictResponse = jsonresponce {
                    ...
                    do{
                        ...
                        self.arrayLogHistory = schedulemodel
    ...
    func callInAction(completion:@escaping (ProcessBot<Any?>)-> Void){
       ...
       // let dictParams:[String:Any] = ["ClientID": clientId]
        ...
        self.manager?.request(.customGetURL(with: .InactionDetails, components: ["ClientID":clientId,"TaskID":"d3a410fa-8fa2-48af-915b-f11cce2f3890"]), method: .get,parameters: nil, encoding: .json, headers: headers, handler:   { (result) in
