...
protocol RobotViewModelProtocol : class {
    ...
    func callStopPauseRobot(taskId:String,status:String, completion:@escaping (ProcessBot<Any?>)-> Void)
    var selecteddirectoryid: String? { get set }
}
    ...
    func getSearchRobotName(completion:@escaping (ProcessBot<Any?>) -> Void){
        if let searchtext = searchRobotString {
            let filteredArray = arrayPublish.filter { (modelObject) -> Bool in
                print(modelObject.friendlyName!)
...
//
//          if (UserDefaults.standard.value(forKey: "SELECTEDDIRECTORYID") as! String) != nil
//          {
//            selecteddirectoryid  = UserDefaults.standard.value(forKey: "SELECTEDDIRECTORYID") as! String
//          }
//         else
//          {
            selecteddirectoryid = "1"
        //  }
        self.manager?.request(.customGetURL(with: .publishedRobot, components: ["ClientID":clientId,"DirectoryID":selecteddirectoryid!,"UserID":userid,"PublishedScriptID":""]), method: .get,parameters: nil, encoding: .json, headers: headers, handler:   { (result) in
...
//        guard let taskId  = UserDefaults.standard.value(forKey: "CLIENTID") else {
//             completion(.failure(ProcessBotError.customMessage("Please try after sometimes")))
//             return
//         }
        let dictParams:[String:Any] = ["TaskID": taskId,"Status":status]
        let headers : HTTPHeaders = [
            "Token": "\(token)",
            "AppName":"IntelgicApp"
        ]
        ...
        self.manager?.request(.stopPauseRobot, method: .post,parameters: dictParams, encoding: .json, headers: headers, handler:   { (result) in
